...
const ImageList = () => {
  ...
  const getImages=()=>{
    axios.get(`http://localhost:5000/images/${JSON.stringify(user._id)}`)
    .then((response)=>{
  ...
  const items= (data.length>0)? data.map((items,key)=>{
    ...
    return(
      <div className='image' onClick={()=>{navigate(`/view/${items._id}`)}}>
     <img src={src} alt=''/>
  ...
  return (
    ...
    <div className='image_add' onClick={()=>{navigate('/add')}}>
